...
const handleSearchChange = (e) => {
 ...
 const filterdUsers = users.filter((user)=>user.name.
  ...
  return (
    <>
      <div className="container">
        <h3>CRUD Application with
           React.js and Node.js

        </h3>
        <div className="input-search">
          <input type="search" placeholder=
          "Search Text Here" onChange={handleSearchChange}/>
          <button className="btn green" 
          onClick={handleAddRecord}>Add Record</button>
        ...
        {isModalOpen && (
          <div className="modal">
            <div className="modal-content">
              ...
              <h2>
                {userData.id ? "User Record" : "Add Record"}</h2>

              <div className="input-group">
                <label htmlFor="name">
                  Full Name</label>
                  <input type="text" value={userData.name} name="name" 
                  id="name" onChange={handleData} />
              </div>

              <div className="input-group">
                <label htmlFor="age">
                  Age</label>
                  <input type="number" value={userData.age} name="age" 
                  id="age" onChange={handleData} />
              </div>
